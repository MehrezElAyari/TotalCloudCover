...
"""
Total cloud coverage project (seasonal)
12-07-2019
Mehrez El Ayari
"""
...
if __name__ == "__main__":
    ...
    for st in Stations:
        ...
        seasonal_dates = [(l.month % 12 + 3) // 3 for l in Data.dates]
        for season in range(1, 5):
            for i in range(StartYear + TrainingSize, LastYear):
                idx = [
                    ((l.year >= (i - TrainingSize)) and (l.year <= i))
                    for l in Data.dates
                ]
                idx_pred = [k.year == (i + 1) for k in Data.dates]
                idx_seasonal = [k == season for k in seasonal_dates]
                idx_train_seasonal = [a and b for a, b in zip(idx, idx_seasonal)]
                idx_pred_seasonal = [a and b for a, b in zip(idx_seasonal, idx_pred)]
                obs_train = observations[idx_train_seasonal, :]
                obs_pred = observations[idx_pred_seasonal, :]
                firstIterationlt = True
                for lt in range(0, 10):
                    f_train = imp.fit_transform(factors[idx_train_seasonal, :, lt])
                    f_pred = imp.fit_transform(factors[idx_pred_seasonal, :, lt])
                    hist = model.fit(
        ...
        robjects.r(
            "save(prob,dates,obs,crps, file='{}')".format("seasonal_prob_" + Data.name)
        )
        del Data
